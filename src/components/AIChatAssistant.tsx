...
import { Send, Bot, User, Loader2, Trash2, Maximize2, Minimize2, X, MessageSquare } from 'lucide-react';
...
export const AIChatAssistant: React.FC<AIChatAssistantProps> = ({ mode = 'page', onClose, onExpand }) => {
  ...
  return (
    <div className={`flex flex-col ${isWidget ? 'h-full w-full' : 'h-[calc(100vh-200px)] max-w-4xl mx-auto rounded-[32px] shadow-2xl border border-zinc-100'} bg-white overflow-hidden`}>
      {/* Header */}
      <div className={`px-6 py-4 bg-[#000080] text-white flex items-center justify-between`}>
        <div className="flex items-center gap-3">
          <div className="p-2 bg-white/10 rounded-xl">
            <Bot size={isWidget ? 20 : 24} />
          </div>
          <div>
            <h3 className={`font-bold ${isWidget ? 'text-sm' : 'text-lg'}`}>LogistMate AI</h3>
